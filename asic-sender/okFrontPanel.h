...
// clang-format off
	...
	typedef uint8_t  UINT8;
		...
		#define okDEPRECATED(msg) [[deprecated(msg)]]
	#endif
#elif defined(_MSC_VER)
	#define okDEPRECATED(msg) __declspec(deprecated)
#endif

#ifndef okDEPRECATED
	#define okDEPRECATED(msg)
#endif
...
okNODISCARD okDLLEXPORT ok_ErrorCode DLL_ENTRY okFrontPanel_GetLastError(okFrontPanel_HANDLE hnd);
okNODISCARD okDLLEXPORT const char* DLL_ENTRY
...
public:
	...
	okNODISCARD ErrorCode GetLastError() const;
...
// This class is deprecated, don't use it in the new code.
...
public:
	...
	okCFrontPanel::ErrorCode List(std::vector<std::string>& names)
	{
		okCDeviceSettingNames settingNames;
		const okCFrontPanel::ErrorCode ec =
			static_cast<okCFrontPanel::ErrorCode>(okDeviceSettings_List(h, settingNames.h));
		if (ec == okCFrontPanel::NoError) {
			const int count = settingNames.GetCount();
			names.resize(static_cast<size_t>(count));
			for (int i = 0; i < count; ++i)
				names[i] = settingNames.Get(i);
		}

		return ec;
	}

	okDEPRECATED("This function only exists for compatibility, use List(std::vector<std::string>&) "
				 "overload instead")
...
okCFrontPanel::GetLastError() const
{
	return (okCFrontPanel::ErrorCode)okFrontPanel_GetLastError(h);
}
inline const char*
